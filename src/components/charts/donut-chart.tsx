...
export function DonutChart({
  ...
  const colorAt = (item: BucketedItem, i: number) => (item.rawKey && colorMap?.[item.rawKey]) || colorForIndex(i);
    ...
    <Card>
      ...
      <CardContent className="flex items-center gap-4">
            ...
            <PieChart>
              ...
              <Pie data={items} dataKey="value" nameKey="label" innerRadius={44} outerRadius={68} paddingAngle={2} strokeWidth={2}>
                {items.map((item, i) => (
                  <Cell
                    key={item.label}
                    fill={item.isOther ? "var(--muted-foreground)" : colorAt(item, i)}
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          {items.map((item, i) => {
            const color = item.isOther ? "var(--muted-foreground)" : colorAt(item, i);
            return (
              <button
                key={item.label}
                type="button"
              ...
              >
                ...
                <span className="h-1.5 min-w-8 flex-1 overflow-hidden rounded-full bg-muted">
                  <span className="block h-full rounded-full" style={{ width: `${(item.value / maxValue) * 100}%`, backgroundColor: color }} />
                </span>
