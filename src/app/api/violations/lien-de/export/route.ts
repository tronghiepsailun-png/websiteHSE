...
// downloaded file comes straight from that template, untouched, so the export can't drift from
// "same as the original file" the way a from-scratch reconstruction inevitably would.
const TEMPLATE_PATH = path.join(process.cwd(), "src/server/templates/work-injury-deduction-template.xlsx");
const TEMPLATE_SHEET_NAME = "2026年8月";
const DATA_COLS = 13; // A..M — 序号 through 罚款金额（越南盾）
const FIRST_DATA_ROW = 4; // rows 1-3 are the title + two-row header
const ORIGINAL_TOTAL_ROW = 12; // where the template's own "合计" row/merge originally sat
...
// rows share the exact same style object by reference. Setting a sub-property (e.g.
// `cell.fill = x`) mutates that shared object in place, silently reformatting every OTHER cell
// that happened to share it too. A fresh `style` object per cell severs the sharing before
// anything gets mutated (see the safety-5s-violation export route, where this was first found).
...
/** Rebuilds the template's own mixed-font title run ("2026" + "年" + "8" + "月份工伤连带责任人扣款名单")
...
function buildTitleRichText(year: number, month: number): ExcelJS.CellRichTextValue {
  const cjk: Partial<ExcelJS.Font> = { bold: true, size: 18, color: { theme: 1 }, name: "等线 Light", charset: 134 };
  const num: Partial<ExcelJS.Font> = { bold: true, size: 18, color: { theme: 1 }, name: "Times New Roman", charset: 134 };
  return {
    richText: [
      { font: num as ExcelJS.Font, text: String(year) },
      { font: cjk as ExcelJS.Font, text: "年" },
      { font: num as ExcelJS.Font, text: String(month) },
      { font: cjk as ExcelJS.Font, text: "月份工伤连带责任人扣款名单" },
    ],
  };
}
...
export async function GET(request: Request) {
  return withApiErrorHandling(async () => {
    ...
    const rows = await listWorkInjuryDeductions(ctx.organizationId, { year, month });
    ...
    // whichever other month sheets happen to exist in the template.
    ...
    // The template's own total row is a merge (A:J) at a fixed row — remove it before writing
    ...
    ws.unMergeCells(`A${ORIGINAL_TOTAL_ROW}:J${ORIGINAL_TOTAL_ROW}`);
    ...
    rows.forEach((row, i) => {
      ...
      excelRow.getCell(11).value = row.accidentDate;
      excelRow.getCell(12).value = row.reporterName ?? "";
      excelRow.getCell(13).value = row.fineAmountVnd;
      applyRowStyle(ws, r, dataStyle);
      excelRow.getCell(11).numFmt = "d/m/yyyy";
      excelRow.height = 37.5;
    });
    ...
    for (let c = 2; c <= 12; c++) totalRow.getCell(c).value = null;
